...
function registrarEmail() {
	swal('Importante', 'Escribe el email de la persona a la que quiere hacer un reporte:', 'info', {
		content: 'input',
	}).then((value) => {
		if (value === '') {
			swal(
				'Advertencia',
				'NO ingreso el email. Descuide, podra hacerlo luego',
				'warning'
			)
		} else {
			if (!validarEmail(value)) {
				...
				swal('Perfecto', 'Email Guardado exitosamente!', 'success')
				emailUser = value
